...
const columns = [
  { id: "district", label: "District Name", minWidth: 170 },
...
export default function ApiPaginationTable() {
  ...
  const fetchData = async (page, rowsPerPage) => {
    try {
      const response = await fetch(
        `https://masterservice.agrozone.in/master/district/v1?searchTerm=&pageNo=${
          page + 1
        }&recordsPerPage=${rowsPerPage}`
      );
      const data = await response.json();

      setRows(data.data.districts || []);
